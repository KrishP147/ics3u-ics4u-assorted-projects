/*
* This is a simple program that generates a random array and finds the number of occurrences of a  specific key from within the array and displays the result to the user using JOPTIONPANE.
...
public class ArrayCountKeyOccurences {
	...
	public static void programDescription(int ARRAY_RANDOM) {//This method displays a simple description of the program to the user.
		JOptionPane.showMessageDialog(null,"This program prompts the user for a key, then returns the number of times it occurs\nin an array of 20 randomly generated numbers from 1 - " + ARRAY_RANDOM + ".");
	}
	...
	public static void displayResult(int[] array, int key) {//this method displays the results - the array and its reverse
		int keyCount = 0; //counts the amount of occurrences for the key
		for(int i=0; i<array.length; i++) { //for the length of the array, if the key is found, the key count is increased
			if (array[i] == key)
				keyCount++;
		} //for	
			String message = "\nKey " + key + " occurs " + keyCount + " time(s) in the array:\n" + Arrays.toString(array) + "!"; //arrays are converted to strings, the key and key count is displayed
	...
	public static void main(String[] args) {
		...
		while (true) {//forever
			displayResult(calculateArray(ARRAY_LENGTH, ARRAY_RANDOM), getKey(ARRAY_RANDOM)); //runs the methods
		}//while
	} //main
}//class
