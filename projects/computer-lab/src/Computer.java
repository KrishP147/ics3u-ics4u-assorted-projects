...
public class Computer {
	...
	// These are the fields or variables of the class. Each DogSimple object will
	...
	// class constructor, invoked with the new operator; instantiate & initializes
	// an object of class Computer, with parameters
	public Computer(String ipAddress, int modelNumber, String operatingSystem, String brand, int manufacturingDate,
			String memorySize, String compatibleIDE) {
	...
	public void makeFrame(JFrame frame, JPanel panel, JPanel keyboard) {
		...
		frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // will dispose frame if x is pressed, going back to
																	// computer lab homepage
		frame.setSize(935, 680);
		frame.getContentPane().setLayout(null);

		JButton googleButton = new JButton("Google"); // button to run browseWeb method
		googleButton.setBackground(new Color(128, 128, 255));
		...
		IDEButton.setBackground(new Color(255, 128, 128));
		...
		IDEButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {// same as above, will set up frame for coding and remove option
														// buttons
				runIDE(frame, panel);
				panel.remove(googleButton);
				panel.remove(IDEButton);
		...
		detailButton.setBackground(new Color(64, 128, 128));
	...
	public void makeKeys(JPanel keyboard) { // manually adding all keys, alligned on keyboard
		...
		buttonUnderline.setForeground(Color.WHITE);
		buttonUnderline.setBackground(Color.BLACK);
		...
		buttonLeftBracket.setForeground(Color.WHITE);
		buttonLeftBracket.setBackground(Color.BLACK);
		...
		buttonRightBracket.setForeground(Color.WHITE);
		buttonRightBracket.setBackground(Color.BLACK);
		...
		buttonSemicolon.setForeground(Color.WHITE);
		buttonSemicolon.setBackground(Color.BLACK);
		...
		buttonColon.setForeground(Color.WHITE);
		buttonColon.setBackground(Color.BLACK);
	...
	public void performSearch(JTextField textField, JFrame frame) { // method to actually do the search in a new tab on
																	// google
		String query = textField.getText().trim(); // query is trimmed and stored - no leading or trailing spaces
		if (!query.isEmpty()) {
			try {
				// encoding the query to handle spaces and other special characters, using
				// standard encoding imports
				query = URLEncoder.encode(query, StandardCharsets.UTF_8.toString());
				String googleSearchUrl = "https://www.google.com/search?q=" + query;// a link is made with the query
				Desktop.getDesktop().browse(new URI(googleSearchUrl)); // new tab is opened with query, using external
																		// desktop class's browse function
			} catch (Exception ex) {
				JOptionPane.showMessageDialog(frame, "Error occurred while searching.", "Error",
						JOptionPane.ERROR_MESSAGE);
				ex.printStackTrace(); // report of errors
			}
		} else { // if nothing is inputted
			JOptionPane.showMessageDialog(frame, "Please enter a search query.", "Warning",
	...
	public void browseWeb(JFrame frame, JPanel panel) { // method to browse web
		...
		textField.addKeyListener(new KeyListener() { // will check for searching if enter key is pressed as well
			...
			@Override // necessary to provide implementations for all the methods here, since
						// KeyListener inherits form an abstract class - must include.
			public void keyTyped(KeyEvent e) {
			} // corresponds to when a key is typed - processing key input - empty, don't need
				// this function.

			@Override
			public void keyReleased(KeyEvent e) {
			} // corresponds to when a key is released - empty, don't need this function.
		});
		...
		backButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				makeFrame(new JFrame(), new JPanel(), new JPanel()); // literally making a new frame from scratch -
																		// corresponding to make frame method
			}
		});
	...
	public void runIDE(JFrame frame, JPanel panel) { // method to run the coding functionality
		...
		JLabel nameLabel = new JLabel(getCompatibleIDE()); // label for header, either eclipse or replit depending on
															// ide attribute
		...
		JButton runButton = new JButton("Run"); // will "run code" simply printing the users input as output to the
												// "console"
		...
		JButton endButton = new JButton("End"); // similar to back button for browseWeb, will close frame and will make
												// new frames and panels
	...
	public void displayComputerInfo() { // method to display info about computers attributes
		String info = getIpAddress() + "\n" + getModelNumber() + "\n" + getOperatingSystem() + "\n" + getBrand() + "\n"
				+ getManufacturingDate() + "\n" + getMemorySize() + "\nThis computer is compatible with "
				+ getCompatibleIDE() + "'s IDE.";
	...
	public static void main(String[] args) { // for use if running independently
		...
		Computer comp = new Computer("192.158.1.38", 1546, "macOS", "LENOVO", 2020, "64GB", "Replit"); // using class
																										// constructor
																										// for
																										// specifications
